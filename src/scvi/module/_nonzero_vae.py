...
class NonZeroVAE(VAE):
    ...
    **kwargs
    ...
    Expected behavior compared to standard SCVI:

    - NLL/gene (nz): Lower (better non-zero prediction)
    - NLL/gene (z): Higher (less focus on zeros)
    - log2(mu/x) bias: Closer to 0 (less under-prediction)
    """
    ...
    def loss(
        ...
        # KL divergence terms (unchanged from standard VAE)
        ...
        # Reconstruction loss: ONLY on non-zero observations
        ...
        if self.normalize_by_nonzero:
            # Normalize by number of non-zero genes per cell
            # This keeps reconstruction on a per-gene scale
            n_nonzero = nonzero_mask.sum(dim=-1).clamp(min=1).float()
            reconst_loss = -masked_log_prob.sum(dim=-1) / n_nonzero
            # Also normalize KL by n_nonzero to maintain consistent KL-to-reconstruction ratio
            # Without this, KL dominates (16 vs ~3) causing posterior collapse
        ...
        # Combine losses
        kl_local_for_warmup = kl_divergence_z_scaled
        kl_local_no_warmup = kl_divergence_l_scaled
